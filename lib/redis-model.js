...
// Set record
RedisModel.prototype.set = function (long_url, short_url, partners, callback) {
  var self = this;
  ...
  this.findUrl(long_url, function (err, reply) {
    if (err) {
      ...
      if(short_url){
        self.findHash(short_url,function(err, reply){
          if(reply){
            callback(500,"Short link already present: "+reply.url);
          }
          else{
            save(short_url);
          }
